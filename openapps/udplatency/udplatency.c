...
void udplatency_task() {
   ...
   p=idmanager_getMyID(ADDR_16B);
   pkt->payload[0]    = p->addr_16b[0];
   pkt->payload[1]    = p->addr_16b[1];
   ...
   //openserial_printStatus(STATUS_UDP_PKT,(uint8_t*)pkt->length,sizeof(pkt->length));
   // send packet
